                ...
                if(path.parent.type !== 'JSXElement'){
                  path.traverse({
                    JSXOpeningElement(path){
                      if(path.node.attributes.some(attr => (attr.name.name === 'block' || attr.name.name === 'elem'))){
                        path.node.attributes = path.node.attributes.filter(attr => !BEMAttrs.has(attr.name.name))
                      }
                    }
                  })
                }
